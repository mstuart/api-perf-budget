...
export function formatResults(routeResults) {
  const lines = ["API Performance Budget Report", "=".repeat(70), ""];

  for (const [route, { measurements, result }] of Object.entries(
    routeResults
  )) {
    ...
    if (result.violations.length > 0) {
      for (const violation of result.violations) {
        lines.push(
          `  VIOLATION: ${violation.metric} = ${violation.actual.toFixed(1)}ms (limit: ${violation.limit}ms)`
        );
      }
    }
